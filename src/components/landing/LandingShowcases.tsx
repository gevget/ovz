...
import { DemoCTA, StatusPill } from "@/components/landing/LandingPrimitives";
...
function ProductScreenContent({ screen, places, clinic, match, routes }: { screen: ProductScreen; places: Place[]; clinic: Place; match: ReturnType<typeof calculateAccessibilityMatch>; routes: RouteOption[] }) {
  ...
  return <div className="space-y-3"><div className="rounded-card bg-[#10233f] p-4 text-white"><p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#a9c6ff]">Journey mode</p><p className="mt-2 text-lg font-bold">До клиники — по шагам</p><p className="mt-1 text-sm leading-6 text-white/70">Следующий шаг: {journey[0]?.title ?? "найти доступный вход"}</p></div><Timeline items={journey.slice(0, 4).map((step, index) => ({ label: step.title, detail: step.detail, current: index === 0 }))} /></div>;
}

export function HeroProductDemo() {
  const { places, clinic, match, routes } = useLandingProductData();
  const [screen, setScreen] = useState<ProductScreen>("map");
  return <div className="relative mx-auto w-full max-w-[520px] lg:mx-0 lg:ml-auto"><PhoneFrame title="Интерактивный экран Навигатора" fullBleedMobile={false}><div className="flex min-h-0 flex-1 flex-col bg-canvas"><div className="flex items-center justify-between border-b border-border bg-surface px-4 py-3"><span className="text-sm font-bold text-ink">Навигатор</span><span className="text-xs font-semibold text-muted">Demo-данные</span></div><div className="flex flex-wrap gap-1 border-b border-border bg-surface px-3 py-2">{(Object.keys(screenLabels) as ProductScreen[]).map((item) => <button key={item} type="button" className={`min-h-9 rounded-full px-3 text-xs font-semibold ${screen === item ? "bg-primary text-white" : "bg-surface-soft text-muted"}`} aria-pressed={screen === item} onClick={() => setScreen(item)}>{screenLabels[item]}</button>)}</div><div className="app-scrollbar min-h-0 flex-1 overflow-y-auto p-3"><ProductScreenContent screen={screen} places={places} clinic={clinic} match={match} routes={routes} /></div></div></PhoneFrame><div className="pointer-events-none absolute -left-5 top-16 hidden rounded-card border border-border bg-surface p-3 shadow-md sm:block lg:-left-28"><p className="text-xs font-semibold text-ink">Вход без ступеней</p><p className="mt-1 text-xs text-success">Доступно</p></div><div className="pointer-events-none absolute -right-5 bottom-28 hidden rounded-card border border-border bg-surface p-3 shadow-md sm:block lg:-right-20"><p className="text-xs font-semibold text-ink">{match.score}% подходит вам</p><p className="mt-1 text-xs text-muted">{clinic.lastConfirmedLabel}</p></div></div>;
}

export function Hero({ onPartner }: { onPartner: () => void }) {
  return <section id="top" className="overflow-hidden border-b border-border bg-canvas"><div className="mx-auto grid min-h-[calc(100vh-64px)] max-w-[1280px] items-center gap-12 px-4 py-14 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-20"><div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-[0.12em] text-primary">Персональный навигатор по городу</p><h1 className="mt-5 text-5xl font-bold leading-[1.02] tracking-[-0.065em] text-ink sm:text-6xl lg:text-7xl">Город и сервисы, которыми действительно можно пользоваться</h1><p className="mt-7 max-w-xl text-lg leading-8 text-muted">Навигатор доступности объединяет доступную инфраструктуру, маршруты, помощь, знания, работу, обучение и сообщества в одном цифровом сервисе. Он учитывает потребности конкретного человека и помогает не только найти место или услугу, но и понять, как ими воспользоваться.</p><div className="mt-8 flex flex-wrap gap-3"><DemoCTA label="Открыть интерактивное демо" /><button type="button" onClick={onPartner} className="inline-flex min-h-12 items-center justify-center rounded-control border border-border bg-surface px-5 text-sm font-semibold text-ink hover:bg-surface-soft">Обсудить партнёрство</button></div><p className="mt-5 text-sm leading-6 text-muted">Demo работает на тестовых данных и показывает ключевые пользовательские сценарии.</p></div><HeroProductDemo /></div></section>;
}
...
export function StickyProductShowcase() {
  ...
  return <div ref={rootRef} className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-start"><div className="space-y-28"><div data-landing-step="map" className="min-h-[24rem] pt-2"><StatusPill tone="primary">01 · Карта</StatusPill><h3 className="mt-5 text-3xl font-bold tracking-[-0.04em] text-ink">Сначала — место и условия.</h3><p className="mt-4 max-w-md text-base leading-7 text-muted">Не просто ближайшая точка, а место, которое можно проверить по своему сценарию.</p></div><div data-landing-step="place" className="min-h-[24rem] pt-2"><StatusPill tone="primary">02 · Карточка</StatusPill><h3 className="mt-5 text-3xl font-bold tracking-[-0.04em] text-ink">Потом — конкретика.</h3><p className="mt-4 max-w-md text-base leading-7 text-muted">Вход, лифт, туалет, подтверждение данных и следующий шаг собраны в одной карточке.</p></div><div data-landing-step="match" className="min-h-[24rem] pt-2"><StatusPill tone="primary">03 · Match</StatusPill><h3 className="mt-5 text-3xl font-bold tracking-[-0.04em] text-ink">Затем — понятное решение.</h3><p className="mt-4 max-w-md text-base leading-7 text-muted">Система показывает персональный процент и не прячет причины за одним зелёным значком.</p></div><div data-landing-step="route" className="min-h-[24rem] pt-2"><StatusPill tone="primary">04 · Маршрут</StatusPill><h3 className="mt-5 text-3xl font-bold tracking-[-0.04em] text-ink">Маршрут выбирается по надёжности.</h3><p className="mt-4 max-w-md text-base leading-7 text-muted">Быстрее — не всегда лучше. В demo первым показывается вариант с лучшей доступностью.</p></div><div data-landing-step="journey" className="min-h-[24rem] pt-2"><StatusPill tone="primary">05 · Journey mode</StatusPill><h3 className="mt-5 text-3xl font-bold tracking-[-0.04em] text-ink">И путь продолжается на месте.</h3><p className="mt-4 max-w-md text-base leading-7 text-muted">Нужный вход, помощь и сообщение о препятствии остаются частью одного сценария.</p></div></div><div className="lg:sticky lg:top-24"><div className="mb-4 flex flex-wrap gap-2 lg:hidden">{(Object.keys(screenLabels) as ProductScreen[]).map((item) => <button key={item} type="button" className={`min-h-10 rounded-full px-3 text-sm font-semibold ${screen === item ? "bg-primary text-white" : "bg-surface-soft text-muted"}`} onClick={() => setScreen(item)} aria-pressed={screen === item}>{screenLabels[item]}</button>)}</div><PhoneFrame title="Сценарий доступного пути" fullBleedMobile={false}><div className="app-scrollbar min-h-0 flex-1 overflow-y-auto bg-canvas p-3"><ProductScreenContent screen={screen} places={places} clinic={clinic} match={match} routes={routes} /></div></PhoneFrame></div></div>;
}
...
export function JourneyShowcase() {
  ...
  const journeyNarrative = [
    ...
    { title: "Проверить доступность", detail: "Открыть условия, Match и дату подтверждения." },
...
export function CommunityShowcase() {
  ...
  return <div className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]"><CommunityCard community={community} joined onJoin={() => undefined} /><PostCard post={post} author={author} liked={false} saved={false} followingAuthor={false} onOpen={() => undefined} onReact={() => undefined} onSave={() => undefined} onShare={() => undefined} onFollow={() => undefined} onOpenProfile={() => undefined} /></div>;
}
...
export function DataFreshnessShowcase() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[1fr_0.85fr] lg:items-center"><div className="relative"><div className="absolute left-7 top-8 hidden h-[calc(100%-64px)] border-l-2 border-dashed border-primary/30 sm:block" aria-hidden="true" />{[{ label: "Пользователь", title: "Лифт не работает", icon: MapPin }, { label: "Партнёр", title: "Обновление карточки", icon: ShieldCheck }, { label: "Контроль качества", title: "Проверка сообщения", icon: CircleCheck }, { label: "Пользователь", title: "Текущий Match", icon: Sparkles }].map(({ label, title, icon: Icon }, index) => <div key={title} className="relative flex gap-4 rounded-card border border-border bg-surface p-5 sm:ml-0"><span className="relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary-soft text-primary"><Icon aria-hidden="true" className="h-4 w-4" /></span><div><p className="text-xs font-semibold uppercase tracking-[0.08em] text-muted">{index + 1} · {label}</p><p className="mt-1 font-bold text-ink">{title}</p></div></div>)}</div><Card className="p-6 sm:p-8"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.08em] text-primary">Объект</p><h3 className="mt-2 text-xl font-bold text-ink">{clinic.name}</h3></div><StateBadge variant={confirmed ? "success" : "verified"} label={confirmed ? "Только что" : clinic.lastConfirmedLabel} /></div><p className="mt-5 text-sm leading-6 text-muted">Доступность меняется. В demo пользователь, организация и команда контроля качества проходят один shared-state сценарий.</p><div className="mt-6 rounded-card bg-surface-soft p-4"><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold text-ink">Подтверждения за период</span><span className="text-xl font-bold text-primary">{partnerAnalytics["30"].accessibilityConfirmations}</span></div><p className="mt-2 text-xs text-muted">Пилотная метрика · Demo-данные</p></div><Button className="mt-5 w-full" variant="secondary" onClick={() => setConfirmed(true)} disabled={confirmed}>{confirmed ? "Данные подтверждены в demo" : "Показать подтверждение"}</Button></Card></div>;
}
...
export function RoleEcosystem() {
  ...
  return <div className="grid gap-8 lg:grid-cols-[0.7fr_1.3fr] lg:items-center"><div className="grid gap-2">{roles.map((role) => <button key={role.role} type="button" onClick={() => setSelected(role)} aria-pressed={selected.role === role.role} className={`flex min-h-16 items-center justify-between rounded-control border px-4 text-left ${selected.role === role.role ? "border-primary bg-primary-soft" : "border-border bg-surface hover:bg-surface-soft"}`}><span className="font-semibold text-ink">{role.label}</span><ArrowRight aria-hidden="true" className="h-4 w-4 text-muted" /></button>)}</div><Card className="border-primary/20 bg-primary-soft p-7 sm:p-10"><p className="text-sm font-semibold uppercase tracking-[0.12em] text-primary">Одна система — разные участники</p><h3 className="mt-4 text-3xl font-bold tracking-[-0.04em] text-ink">{selected.label}</h3><p className="mt-4 max-w-xl text-lg leading-8 text-muted">{selected.description}</p><div className="mt-7 flex flex-wrap gap-3"><DemoCTA label={`Открыть demo как ${selected.label.toLowerCase()}`} role={selected.role} /></div></Card></div>;
}
