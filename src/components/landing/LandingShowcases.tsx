...
export function Hero({ onPartner }: { onPartner: () => void }) {
  return <section id="top" className="surface-grid overflow-hidden border-b border-border bg-canvas"><div className="page-container grid items-center gap-8 py-8 lg:grid-cols-[0.92fr_1.08fr] lg:py-10"><div className="min-w-0 max-w-2xl"><p className="text-sm font-bold uppercase tracking-[0.12em] text-primary">Персональный навигатор по городу</p><h1 className="text-balance mt-4 break-words text-4xl font-bold leading-tight tracking-[-0.065em] text-ink sm:text-6xl lg:text-6xl">Город и сервисы, которыми действительно можно пользоваться</h1><p className="mt-5 max-w-xl break-words text-lg leading-8 text-muted">Навигатор доступности объединяет доступную инфраструктуру, маршруты, помощь, знания, работу, обучение и сообщества в одном цифровом сервисе. Он учитывает потребности конкретного человека и помогает не только найти место или услугу, но и понять, как ими воспользоваться.</p><div className="mt-6 flex flex-wrap gap-3"><DemoCTA label="Открыть интерактивное демо" /><button type="button" onClick={onPartner} className="inline-flex min-h-12 items-center justify-center rounded-control border border-border bg-surface px-5 text-sm font-semibold text-ink hover:bg-surface-soft">Обсудить партнёрство</button></div><p className="mt-3 break-words text-sm leading-6 text-muted">Демонстрация работает на тестовых данных и показывает ключевые пользовательские сценарии.</p></div><div className="min-w-0"><HeroProductDemo /><div className="mt-4 rounded-card border border-accent/20 bg-accent-soft p-4"><p className="text-xs font-bold uppercase tracking-[0.1em] text-accent">Город как маршрут</p><p className="mt-2 text-sm font-semibold leading-6 text-ink">Проверяем не только точку на карте, но и путь до неё.</p></div></div></div></section>;
}
