...
export function AppScreenHeader({ title, backHref = routes.demo, onBack, rightAction, compact = false, eyebrow }: AppScreenHeaderProps) {
  ...
  return (
    <header className={cn("relative z-10 flex min-h-[4.25rem] items-center justify-between gap-3 border-b border-border/80 bg-surface/95 px-5 shadow-xs backdrop-blur", compact ? "py-3" : "pb-3 pt-6 max-md:pt-4")}>
      ...
      {rightAction ? <div className="min-w-0 max-w-[42%] shrink-0">{rightAction}</div> : null}
    </header>
  );
}
