...
export function RouteOptionCard({ route, recommended = false, onSelect }: { route: RouteOption; recommended?: boolean; onSelect: (routeId: string) => void }) {
  return (
    <Card className={`p-4 ${recommended ? "border-primary ring-2 ring-primary/10" : ""}`}>
      <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-bold text-ink">{route.title}</h2>{recommended ? <Badge className="bg-success-soft text-success">Рекомендуем</Badge> : null}</div><p className="mt-1 text-sm text-muted">{route.accessibilityReliability * 100}% надёжность доступности</p></div><CheckCircle2 aria-hidden="true" className={`h-5 w-5 ${recommended ? "text-success" : "text-border"}`} /></div>
