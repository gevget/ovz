...
export function RequestStatusScreen({ requestId }: { requestId: string }) {
  ...
  const timeline = sequence.map((status, index) => ({ label: labels[status], detail: index <= currentIndex ? (index === currentIndex ? "Текущий статус" : "Шаг пройден") : "Следующий шаг", current: index === currentIndex }));
