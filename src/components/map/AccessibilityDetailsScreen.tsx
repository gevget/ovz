...
export function AccessibilityDetailsScreen({ placeId }: { placeId: string }) {
  ...
  if (!place) return <div className="flex min-h-0 flex-1 flex-col"><MapHeader title="Условия не найдены" backHref={routes.user.map} /><main className="p-5"><EmptyState title="Объект не найден в demo-данных" action={<Link href={routes.user.map} className="inline-flex min-h-11 items-center rounded-control bg-primary px-4 text-sm font-semibold text-white">Вернуться к карте</Link>} /></main></div>;
