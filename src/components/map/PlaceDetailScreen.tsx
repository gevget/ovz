...
export function PlaceDetailScreen({ placeId }: { placeId: string }) {
  ...
  if (!place) return <div className="flex min-h-0 flex-1 flex-col"><MapHeader title="Место не найдено" backHref={routes.user.map} /><main className="app-scrollbar flex-1 overflow-y-auto p-5"><EmptyState title="Объект не найден в тестовых данных" description="Вернитесь к карте и выберите место из локального набора." action={<Link href={routes.user.map} className="inline-flex min-h-11 items-center rounded-control bg-primary px-4 text-sm font-semibold text-white">Вернуться к карте</Link>} /></main></div>;
