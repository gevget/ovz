  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-canvas px-5 py-12 text-ink">
      <Card className="w-full max-w-lg p-6 text-center sm:p-8">
        ...
        <p className="mt-3 text-base leading-7 text-muted">Проверьте адрес или вернитесь к началу demo-сценария.</p>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          ...
          <Link href="/demo" className="inline-flex min-h-11 items-center gap-2 rounded-control bg-primary px-4 text-sm font-semibold text-white hover:bg-[#1f5ed9]"><ArrowLeft aria-hidden="true" className="h-4 w-4" />Открыть demo</Link>
        </div>
      </Card>
    </main>
  );
